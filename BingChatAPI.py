...
class BingChatAPI(LLM):
    ...
    def select_conversation(self, conversation_style: str):
        if conversation_style == "precise":
            ...
            raise ValueError(
                "conversation_style must be precise, creative or balaced")
    ...
    async def call(self, prompt: str, stop: Optional[List[str]] = None) -> str:
        ...
        if self.chatbot is None:
            if self.cookiepath is None:
                raise ValueError(
                    "Need a COOKIE , check https://github.com/acheong08/EdgeGPT/tree/master#getting-authentication-required for get your COOKIE AND SAVE")
            else:
                # if self.chatbot == None:
                self.chatbot = await Chatbot.create(cookie_path=self.cookiepath)

        if self.conversation_style is not None:
            self.conversation_style_on = self.select_conversation(
        ...
        if response_text == "":
            ...
            print(">>>> [DEBBUGGER] hidden_text = " +
                  str(hidden_text) + " [DEBBUGGER] <<<<")
            print(
                ">>>> [DEBBUGGER] BING CHAT dont is open Like CHATGPT , BingCHAT have refused to respond. [DEBBUGGER] <<<<")
